...
export function getResetPasswordHtml(resetUrl: string): string {
  const content = `
    ...
    ${emailText("We received a request to reset your DollarShop account password. Click the button below to choose a new one.")}
  ...
  return emailLayout(content, "Reset your DollarShop password");
}
