...
export function ShareButton({ productName, productSlug }: ShareButtonProps) {
  ...
  const supportsNativeShare = useSyncExternalStore(subscribe, getSnapshot, getServerSnapshot);
  ...
  async function handleNativeShare() {
    try {
      await navigator.share({ title: productName, text, url: getUrl() });
    } catch {
      // user cancelled
    }
  }
    ...
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-48">
        <DropdownMenuItem onClick={handleCopyLink}>
          <span className="mr-2">{copied ? <CheckIcon /> : <CopyIcon />}</span>
          Copy link
        </DropdownMenuItem>
